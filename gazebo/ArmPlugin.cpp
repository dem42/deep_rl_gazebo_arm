...
namespace gazebo
{
  ...
  {
    //if(DEBUG){printf("Updating joints\n");}
  ...
  void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
  {
    ...
    //if(DEBUG){printf("Before inter %d, %d, %d, %d\n", hadNewState, newReward, testAnimation, newState);}
    
    // if an EOE reward hasn't already been issued, compute an intermediary reward
    if( hadNewState && !newReward )
      {
	if(DEBUG){printf("Intermediary reward\n");}
	...
	if(checkGroundContact)
	  {
	    ...
	    if(DEBUG){printf("GROUND CONTACT, EOE %f, %f\n", gripBBox.min.y, gripBBox.min.z);}
	...
	*/ 
	...
	if(!checkGroundContact)
	  {
		...
		avgGoalDelta  = avgGoalDelta * SMOOTH_REWARD_ALPHA + (1-SMOOTH_REWARD_ALPHA)*distDelta;
	...
	if( endEpisode )
	  {
	    if(DEBUG){printf("ending the episode\n");}
